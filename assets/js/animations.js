...
(function () {
	...
	document
		.querySelectorAll(
			'.une-photo img,.galerie img,.pole img,.carte-actu img,.membre img,.grande-actu img,.film>img,.projet-photo img'
		)
	...
	var cibles =
		'.tuile,.gal-accueil figure,.gal-note,.chiffres>div,.tete,.mission>*,.impact>div,.domaine,.ligne-rep,.etape,.actu,.temoin,.partenaires,.faq details,.appel .enveloppe>*,.contact>*,.duo>*,.trio>*,.galerie .cadre,.pole,.membre,.carte-actu,.grande-actu>*,.bloc,.aside .encadre,.revue a,.bloc-legal,.rejoindre,.film .sur>*,.bandeau figcaption';
	...
	// # Compteurs
	var compteurs = [].slice.call(
		document.querySelectorAll(
			'.chiffres .v,.impact .c,.transp .gros,.encadre .annee,.gal-note .n'
		)
	);
	...
	function compte(el) {
		...
		function pas(t) {
			...
			el.textContent =
				avant +
				fmt(Math.round(cible * (1 - Math.pow(1 - p, 3)))) +
				apres;
	...
	function bouge() {
		bandeaux.forEach(function (img) {
			...
			var p =
				...
				window.innerHeight;
			img.style.transform =
				'translate3d(0,' + (-p * 26).toFixed(1) + 'px,0) scale(1.08)';
		});
	}
	...
	var minuterie = setInterval(function () {
		verifie();
		if (!aVoir.length && !compteurs.length && !barres.length)
	...
	// # Affichage forcé après 4 secondes
	setTimeout(function () {
		if (
			aVoir.some(function (el) {
				return visible(el, 0);
			})
		)
			tout();
